...
print("問題のある記事:閲覧数",result_ran_view)
print("問題のある記事:編集数",result_ran_edit)


with open(r'c:/Users/KoyanagiKazuya/Desktop/12-9/sample_writer_row11.csv', 'w', newline="") as f:
    writer = csv.writer(f)

    writer.writerow(['point',"view","edit","NofItem","TexNum","isbn","url"])

    for item in range(num+1):#これだと499
       print(item)
       writer.writerow(['0',result_ran_view[item] ,result_ran_edit[item] ,result_ran_nofitem[item] ,result_ran_texnum[item],result_ran_isbns[item],result_ran_urls[item]   ])

with open(r'c:/Users/KoyanagiKazuya/Desktop/12-9/sample_writer_row11.csv') as f:
    print(f.read())
